...
#define PY_STDOUT_STDERR_FILE "../log/stdout_stderr.log"

int redirect_output(void)
{
    int fd = open(PY_STDOUT_STDERR_FILE, O_CREAT | O_WRONLY | O_APPEND, 0666);
    if (fd == -1) {
        NGLOG_ERROR("open %s failed, [%m].", PY_STDOUT_STDERR_FILE);
        return -1;
    }
...
void check_output(void)
{
    ...
    if (now != last_check_time)
    {
        ...
        if (off > 10*1024*1024) {
            truncate(PY_STDOUT_STDERR_FILE, 0);
        }
...
extern "C" int spp_handle_init(void* arg1, void* arg2)
{
    ...
    if (base->servertype() == SERVER_TYPE_WORKER) // WORKER进程初始化
    {
        ...
        if (srpc_py_init())
        {
             NGLOG_ERROR("Python init failed.");
                return -3;
        }

        // 载入entry文件
        if (srpc_py_load_file())
        {
            NGLOG_ERROR("Load entry.py failed.");
            return -3;
        }

        // 调用py初始化函数
        if (srpc_py_handler_init(etc))
        {
            NGLOG_ERROR("Py service init failed.");
            return -4;
        }

        //TODO
        //srpc_py_set_service_name(base->servicename().c_str(), (int)(base->servicename().size() + 1));
    }

    NGLOG_INFO("spp_handle_init py success.");
    return 0;
}
...
extern "C" int process_http(unsigned flow, void* arg1, void* arg2)
{
    ...
    // 6. 调用处理函数
    //TODO
    //srpc_py_set_header(&rpc_head);
    ret = srpc_py_handler_process(rpc_head.method_name(), extinfo, 2, body, rsp_body);
    if (ret != SRPC_SUCCESS)
    {
...
extern "C" int spp_handle_process(unsigned flow, void* arg1, void* arg2)
{
    ...
    // 4. 调用处理函数
    ret = srpc_py_handler_process(rpc_head.method_name(), extinfo, 1, req_body, rsp_body);
    if (ret != SRPC_SUCCESS)
    {
        RPC_REPORT(errmsg(ret));
        rpc_head.set_err(ret);
