...
string g_srpc_service_name = "python.client";
...
int srpc_py_header_init(CRpcHead &head, const char *method_name, uint64_t seq)
{
...
int srpc_py_pack(const char *method_name, uint64_t seq, const void *body, int body_len, char *buf, int32_t *plen)
{
    ...
    ret = srpc_py_header_init(head, method_name, seq);
    if (ret < 0)
    {
        return ret;
    }
