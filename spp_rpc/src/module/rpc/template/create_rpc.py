...
def ParseProtoFile(filename):
    ...
    pattern = re.compile(r'package\s+(\w+);', re.M)
...
def ReplaceCode(org_str, module, service, method, request, response):
    """ """
    org_str = re.sub(r'\$\(MODULE_REPLACE\)', module, org_str, re.S | re.M)
...
def ProcessService(dict_pb, inputfile, outputfile):
    ...
    for (key,value) in dict_pb['services'].items():
        ...
        method_imp = ""
        for item in value:
            method_def = method_def + ReplaceCode(mth_def, dict_pb['module'], item[0], item[1], item[2], item[3])
            method_imp = method_imp + ReplaceCode(mth_imp, dict_pb['module'], item[0], item[1], item[2], item[3])
...
def GenerateRpcServer(pb, output_path):
    ...
    ProcessService(pb, template_path + 'service_demo_impl.hpp', output_path + "service_%s_impl.hpp" % pb['module'])
    ProcessMsgHead(pb, template_path + 'msg_demo_impl.h', output_path + "msg_%s_impl.h" % pb['module'])
    ProcessMsgImpl(pb, template_path + 'msg_demo_impl.cpp', output_path + "msg_%s_impl.cpp" % pb['module'])
    ProcessFrame(pb, template_path + 'service_demo_frm.cpp', output_path + "service_%s_frm.cpp" % pb['module'])
...
def ProcessClientFrame(pb, inputfile, outputfile):
    ...
    for (key,value) in pb['services'].items():
        for item in value:
            file_content = ReplaceCode(file_content, pb['module'], item[0], item[1], item[2], item[3])
            open(outputfile, 'w').write(file_content)
            return
...
def GenerateRpcClient(pb, output_path):
    ...
    ProcessClientFrame(pb, template_path + 'client_demo_sync.cpp', output_path + "client_%s_sync.cpp" % pb['module'])
...
if __name__ == '__main__':
    ...
    ## check output path
    output_path = os.path.abspath(sys.argv[3]) + '/' + pb["module"]
    if not os.path.exists(output_path):
        os.makedirs(output_path)

    ## check output svr path
    server_path = output_path + '/' + pb["module"] + "_server/"
    if not os.path.exists(server_path):
        os.makedirs(server_path)
    os.system("cp %s %s" % (sys.argv[1], server_path))

    ## client path
    client_path = output_path + '/' + pb["module"] + "_client/"
