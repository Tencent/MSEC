...
public class MonitorAttrContDays extends JsonRPCHandler {
    ...
    public OneAttrDaysChart exec(MonitorRequest request)
    {
        ...
        if(request.getService_name().equals("") && !request.getServer_ip().equals("")) {
            request.setService_name("RESERVED.monitor");    //get IP info
        }
        ...
        try {
            ...
            ArrayList<OneDayValue> values = MonitorBySvcOrIP.getAttrDaysValue(request.getService_name(),
                    ...
                    dateList);
            if(request.getServer_ip()!=null && request.getServer_ip().length() > 0 && !request.getServer_ip().contains(":"))    //host uses attribute as title
