...
int CGetProcCenter::GetValueData(const string& servicename, const string& attrname, const string& ip, uint32_t day, char* valuedata)
{
	...
	if(!ip.empty())
	{
		...
		if(npos != string::npos)
		{
			numeric_ip = tce::InetAtoN(ip.substr(0, npos));			
			if(numeric_ip == 0)
			{
			...
			else
			{
				...
				else
				{
					...
					for(int i = 0; i < servicedata.ips_size(); i++)
					{
						...
						msec::monitor::HostData* hostdata = service->add_hosts();
						hostdata->set_ip(tce::InetNtoA(servicedata.ips(i).ip()));
						for(int j = 0; j < servicedata.ips(i).ports_size(); j++)
						{
							hostdata->add_ports(servicedata.ips(i).ports(j));
						}
					}
