...
public:
	...
	int UpdateValueData(const msec::monitor::Attr& attr, const string& host, const string& ip, int day, int begin, int end, int index, bool needjudge);
...
private:
	...
	monitor::CMySql mysql_redis;
	std::tr1::shared_ptr<map<string, string> > host_servicenames;
};
