...
int LogsysApi::Init(const char* conf_file)
{
    ...
    //读取日志级别配置
    std::string log_level = inifile.GetValue(GROUP_LOG, "Level", "");
    ...
    for (i = 0; i < 1000; ++i)
    {
        ...
        color_field_name = inifile.GetValue(GROUP_COLOR, color_conf_name, "");

        if (color_field_name.empty())
            break;

        snprintf(color_conf_name, sizeof(color_conf_name), "FieldValue%d", i);
        color_field_value = inifile.GetValue(GROUP_COLOR, color_conf_name, "");
