...
if test "$PHP_LOG_PHP" != "no"; then
  ...
  PHP_NEW_EXTENSION(log_php, log_php.cpp srpc_cintf.cpp srpc_log.cpp, $ext_shared)
fi

PHP_ADD_INCLUDE(../src)
LDFLAGS="$LDFLAGS -L../src -L../third_party/protobuf/lib"
if test "$PHP_PLAT_64" != "yes"; then
LDFLAGS="$LDFLAGS -llogsys_api -lprotobuf -lpthread"
else
LDFLAGS="$LDFLAGS -llogsys_api -lprotobuf -lpthread"
fi
