...
PHP_FUNCTION(nglog_set_option)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss", &key_str, &key_len, &val_str, &val_len) == FAILURE) {
        return;
    }

    set_log_option_str(key_str, val_str);
}
...
PHP_FUNCTION(get_config)
{
    ...
    IniFile  inifile;
    int ret = inifile.load(filename);
    if (ret != RET_OK)
    {
        RETURN_NULL();
    }

    std::string val_str = inifile.getStringValue(session, key, ret);
    val = (char*)val_str.c_str();
