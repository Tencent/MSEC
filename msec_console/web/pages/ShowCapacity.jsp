                      ...
                      if (status == "success") {//http通信返回200
                          if (data.status == 0) {//业务处理成功
                              ...
                              $.each(data.capacity_list, function (i, rec) {
                                  ...
                                  str += "<td>" +rec.load_level  + "</td><td>" + rec.ip_port_count+"</td>";
                                  str += "<td>";
                                  str +="<a href='#' onclick='change_capacity(\"" + rec.first_level_service_name+"\",\""+rec.second_level_service_name + "\")'> 扩缩容</a>";
                                  str+="</td>";
                                  str += "</tr>\n";
                              });
...
<div id="title_bar" class="title_bar_style">
  <p>&nbsp;&nbsp;容量管理  todo:这里看怎么更好的展现LB的管理和信息</p>
</div>
...
<table class="table table-hover" >
  <thead>
    <td>业务</td><td>负载容量百分比</td><td>IP:port数</td><td>操作</td>
  </thead>
  <tbody id="my_table">
  </tbody>
</table>
